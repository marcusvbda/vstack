<template>
    <div :class="`row crud-type-${crud_type.template} view-only`" id="crud-view">
        <div class="col-12">
    ...
    methods: {
        ...
        submit(clicked_btn = "save_and_back") {
            const actions = {
                ...
                back: () => {
                    this.$loading({ text: "Aguarde ..." });
                    window.location.href = this.redirect.replace(`/${this.code}`, "");
                }
            };
            actions[clicked_btn] ? actions[clicked_btn]() : console.log("Action não existe");
        }
    }
};
</script>
