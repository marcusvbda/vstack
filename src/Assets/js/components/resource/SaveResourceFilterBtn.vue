        ...
        <div v-for="(r, i) in reports" :key="i">
            <div class="report-card-option">
                ...
                <el-button
                    @click="openReport(r)"
                    plain
                    :type="isCurrentReport(r) ? 'primary' : ''"
                >
                    {{ r.title }}
                </el-button>
            </div>
...
export default {
    ...
    computed: {
        can_create() {
            return (
                this.reports.length < this.limit &&
                !this.filter_exists &&
                this.has_filter
            );
        },
        filter_exists() {
            return (
                this.reports.filter((x) => this.isCurrentReport(x)).length > 0
            );
        },
        has_filter() {
    ...
    methods: {
        ...
        openReport(report) {
            if (this.isCurrentReport(report))
                return this.$message('Este é o relatório atual');
            this.$loading({ text: 'Carregando modelo de relatório ...' });
            window.location.href = window.location.pathname + report.query;
        },
        deleteReport(index) {
            this.$confirm(
                `Deseja excluir este modelo de filtro ?`,
                'Confirmação',
        ...
        create() {
            this.$prompt(
                'Digite o nome do modelo de relatório',
                'Salvar Modelo de Relatório',
                {
                    ...
                    inputValidator: (input) => {
                        ...
                        if (
                            this.reports.filter((x) => x.title == input)
                                .length > 0
                        )
                            return 'Já existe um modelo com o mesmo nome';
                        return true;
                    },
                }
            ).then(({ value }) => {
        ...
        saveReports(_reports) {
            this.$http
                .post(
                    `${window.location.pathname}/create-report-template`,
                    _reports
                )
                .then(({ data }) => {
                    this.loading = false;
