...
export default {
    ...
    methods: {
        getName(cloned_row) {
            if (cloned_row.data.status == 'exporting')
                return this.getExportingGif();
        ...
        getMicrotime(cloned_row) {
            if (cloned_row.data.status == 'exporting')
                return this.getExportingGif();
            if (
                !cloned_row?.data?.microtime?.start?.start &&
                !cloned_row?.data?.microtime?.end
            )
                return ` - `;
            let time = parseInt(
                cloned_row.data.microtime.end - cloned_row.data.microtime.start
            );
        ...
        createListeningStatusChange() {
            ...
            if (laravel.user.id && laravel.chat.pusher_key) {
                this.$echo
                    .private(`App.User.${laravel.user.id}`)
                    .listen(
                        `.notifications.exporting_status.${this.row.id}`,
                        (n) => {
        ...
        getDueDate(report) {
            if (!['ready', 'error'].includes(report.data.status))
                return this.getExportingGif();
        ...
        getExportingGif() {
            return `
				<div class="flex  align-center py-2">
					<div class="spinner-grow spinner-grow-sm text-neutral-400 mr-2" style="zoom:.5;" role="status">
        ...
        getUrlContent(r) {
            if (r.data.status == 'ready')
                return `<a target='_BLANK' href='${r.data.route}'>Clique aqui efetuar o download</a>`;
            if (r.data.status == 'exporting') return this.getExportingGif();
            if (r.data.status == 'error')
                return `<span class='text-danger'><b class="mr-2">Erro :</b>${r.data.error_message}</span>`;
