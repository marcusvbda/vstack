        ...
        <template v-for="(row, i) in rows">
            <tr :key="i" :id="`resource-body-${row.content.id}`">
                <td v-for="(col, y) in columns" :key="`${i}_${y}`" :id="`resource-body-${col}`">
                    <resource-tablelist-allinone-item :virtual_indexes="virtual_indexes" :row="row" :col="col"
                        :first_key="first_key == col" :resource_id="resource_id" :resource_route="resource_route">
                        <template v-slot:after_row="{ after_row, after_row_content }">
                            <template v-if="row.content.after_row">
                                <el-tooltip class="item" effect="dark"
                                    :content="`${after_row.visible ? 'Ver Menos' : 'Ver Mais'}`" placement="top">
                                    <span class="el-icon-arrow-down arrow-after-icon" v-if="!after_row.visible"
                                        @click="after_row.visible = true" />
                                    <span class="el-icon-arrow-up arrow-after-icon" v-else
            ...
            <tr class="table-row after" :key="`table_after_row_${i}`" :id="`resource-body-${row.content.id}-after-row`">
...
export default {
    props: ["rows", "table_keys", "has_actions", "show_right_actions_column", "resource_id", "resource_route"],
    data() {
        return {
