<template>
    ...
    >
        ...
        <template v-if="loading_items">
            <div
                class="tree-view-item py-0"
                v-for="(i, ix) in $randomInt(3, 5)"
                :key="i"
            >
                <div class="tree-view-label">
                    <div
                        class="shimmer resource-tree-item"
                        :style="{ width: `${$randomInt(10, 50)}%` }"
                    />
            ...
            <template v-if="items.length">
                ...
                >
                    <ul class="tree-view-label hoverable item striped-list">
                        <li class="w-full">
                            <div class="flex align-center w-full no-margin">
                                <VRuntimeTemplate
                                    v-if="template"
                                    :key="i"
                                ...
                                >
                                    <ElButton
                                        v-if="
                                            input.acl.delete || input.acl.update
                                        "
                    ...
                    <div class="my-3" v-for="(child, i) in children" :key="i">
                        <TreeView
                            ...
                            :label_index="
                                child.label_index ? child.label_index : 'name'
                            "
            ...
            <template v-else>
                <div class="my-4 flex align-center justify-center">
                    <small class="text-neutral-400">
                        {{
                            filter
                                ? 'Nada encontrado, revise seu filtro ...'
                                : `Nada relacionado ...`
                        }}
                    </small>
                </div>
...
export default {
    ...
    methods: {
        ...
        loadItems(ignore_loaded = false) {
            ...
            this.$http
                .get(`${this.route_load}/load-items`, payload)
                .then(({ data }) => {
                    this.loaded = true;
                    this.items = data;
                    this.loading_items = false;
