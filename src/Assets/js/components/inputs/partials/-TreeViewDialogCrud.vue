<template>
    ...
    >
        ...
        <template v-if="loading">
            <div class="card-body" style="padding: 13px">
                <div class="flex flex-col">
                    <div
                        v-for="(i, ix) in qtyfields"
                        :key="ix"
                        class="shimmer resource-tree-item-input w-full"
                    />
                </div>
            </div>
            <div class="card-footer">
                <div class="flex justify-between">
                    <div
                        class="shimmer btn-crud-item delete w-25"
                        v-if="selected.id && acl.delete"
                    />
                    <div
                        class="shimmer btn-crud-item w-25 ml-auto"
                        v-if="
                            (selected.id && acl.update) ||
                            (!selected.id && acl.create)
                        "
                    />
                </div>
            </div>
        ...
        <template v-else>
            <div
                class="card-body"
                :style="{ padding: `${cards.length > 1 ? '0 13px' : '0'}` }"
            >
                <form
    ...
    methods: {
        ...
        destroy() {
            this.$confirm('Deseja excluir este registro ?', 'Confirmação', {
                ...
                this.$http
                    .post(
                        `/admin/${this.resource}/${this.selected.code}/destroy`,
                        {
        ...
        init() {
            ...
            this.$http
                .get('/admin/inputs/resource-tree/load-crud', payload)
                .then(({ data }) => {
                    this.cards = data;
                    this.initFields();
                    this.loading = false;
