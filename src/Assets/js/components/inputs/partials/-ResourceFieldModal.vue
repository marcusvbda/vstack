                ...
                <div class="card h-100 w-full">
                    <div class="card-header modal-header">
                        <div class="flex justify-between align-center w-full">
                            <template v-if="rendered_data.can_update">{{
                                form.id > 0
                                    ? `Edição de ${rendered_data.singular_label}`
                                    : `Cadastro de ${rendered_data.label}`
                            }}</template>
                            ...
                            <div class="col-12">
                                ...
                                >
                                    ...
                                    <div
                                        class="row"
                                        v-if="
                                            (rendered_data.can_update &&
                                                form.id) ||
                                            (rendered_data.can_create &&
                                                !form.id)
                                        "
                                    >
                                        <div
                                            class="col-12 flex justify-between flex align-center flex-wrap"
                                        >
                                            <div>
                                                <button
                                                    v-if="
                                                        rendered_data.can_delete &&
                                                        form.id
                                                    "
                                                    @click="destroy"
        ...
        submit() {
            this.$confirm(`Deseja Salvar ?`, 'Confirmação', {
                ...
                .then(() => {
                    ...
                    this.$http
                        .post(this.rendered_data.store_route, this.form)
                        .then((res) => {
                            let data = res.data;
                            if (data.message)
                                this.$message({
        ...
        destroy() {
            this.$confirm(`Confirma Exclusão ?`, 'Confirmação', {
                ...
                .then(() => {
                    ...
                    this.$http
                        .delete(
                            this.rendered_data.destroy_route.replace(
                                '_replace_area_',
                                this.form.id
                            ),
                            {}
