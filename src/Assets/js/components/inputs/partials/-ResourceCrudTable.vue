    ...
    <div>
        <div class="table-responsive-sm" v-if="rendered_data.data_count > 0">
            <table class="table table-striped hovered resource-table table-hover mb-0">
                <thead>
                    <tr>
                        <th v-for="(value, i) in rendered_data.table" :key="i">{{ value['label'] ? value['label'] : value }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(data, d) in rendered_data.data" :key="d">
                        <template v-for="(value, i, x) in rendered_data.table">
                            <td :key="i" v-if="!(x == 0 || value == 'name')"><v-runtime-template :template="getTableValue(data, i, value, x)" /></td>
                            <td :key="i" v-else>
                                <a href="#" class="link" @click.prevent="edit(data)" v-html="getTableValue(data, i, value, x)"></a>
                            </td>
                        </template>
                    </tr>
                </tbody>
