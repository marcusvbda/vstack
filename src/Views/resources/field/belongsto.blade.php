<v-select v-model='{{ 'form.' . $field }}' label='{{ $label }}' :disabled='{{ $disabled }}'
    ...
    :option_template='@json($option_template)' :multiple='@json($multiple)'
    route_list='{{ $route_list }}' field_index="{{ $field }}" :option_list='{{ $options }}'
    :model_filter='{{ $model_filter }}' :allow_create='{{ $allow_create }}' entity_parent='{{ $entity_parent }}'
