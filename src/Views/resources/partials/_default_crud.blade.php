...
<resource-crud class="mt-2" :data="{{ json_encode($data) }}" :params="{{ json_encode($params) }}" redirect="{{ $current_route }}" :breadcrumb="{{ json_encode($routes) }}" @if (@$content) :content="{{ json_encode($content) }}" @endif raw_type='{{ $raw_type }}' :first_btn='@json($resource->firstCrudBtn())' :second_btn='@json($resource->secondCrudBtn())' :acl='@json(["can_update" => $resource->canUpdate()])' :has_befores_store='@json($resource->beforeStore([]) !== false)' loading_message="{{$resource->resourceLoadingSaveMassage($raw_type)}}">
    @if (@!$data->id && !@$data['id'])
