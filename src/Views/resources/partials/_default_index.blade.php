<div class="row mb-3 mt-2">
    <div class="col-12 d-flex flex-row align-items-center sm-flex-wrap" data-aos="fade-left">
        ...
        <div class="d-flex flex-row  flex-wrap align-items-center sm-w-100 sm-mt-4">
            ...
            @if ($resource->model->count() > 0)
            @php
            $resource_config_query = \marcusvbda\vstack\Models\ResourceConfig::where('data->user_id',
            $user->id)->where('resource', $resource->id);
            $config_columns = (clone $resource_config_query)->where('config',
            'resource_export_disabled_columns')->first();
