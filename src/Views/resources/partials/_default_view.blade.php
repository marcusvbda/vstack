...
<resource-view :data="{{ json_encode($data) }}" :params="{{ json_encode($params) }}" redirect="{{ $current_route }}" data-aos="fade-right"
    :breadcrumb="{{ json_encode($routes) }}" @if (@$content)
    :content="{{ json_encode($content) }}"
    @endif
    right_card_content="{{ $resource->crudRightCardBody() }}"
    raw_type='{{ $raw_type }}'
    :first_btn='@json($resource->firstViewBtn())'
    :second_btn='@json($resource->secondViewBtn())'
    :acl='@json(["can_update" => $resource->canUpdate() && $resource->canUpdateRow($row)])'
